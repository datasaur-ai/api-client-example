...
def coco_to_datasaur_schemas(coco_object: COCO) -> List[DatasaurSchema]:
    """
    One COCO JSON contains many images, hence the return value of this function needs to be a list of objects
    Since the return value is a list of Python dataclass object, we need to use dataclasses.asdict before json.dump-ing it. See the main() function for an example.
    """
...
def main() -> None:
    ...
    with open(sample_coco) as f:
        ...
        coco_obj = from_dict(data=json_data, data_class=COCO)

    schemas = coco_to_datasaur_schemas(coco_obj)
